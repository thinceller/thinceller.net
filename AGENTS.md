...
Next.js 16.2.1 (App Router) を使用した個人サイト（ブログ機能含む）です。
Vercelにデプロイされ、日本語技術記事に特化した機能を持ちます。

## セットアップコマンド

**開発環境:** Nix Flakes + direnv（`flake.nix` で Node.js 22.x / pnpm を管理）
**パッケージマネージャー:** pnpm@10.33.0（バージョンは `package.json` の `packageManager` フィールドで管理）
...
- `components/FaviconLink.tsx`: Faviconリンク管理コンポーネント
...
- **Tailwind CSS 4.2.2** + カスタムタイポグラフィ
- **Shiki 4.0.2** シンタックスハイライト（@shikijs/* パッケージ）
- **Biome 2.4.9** コード品質管理
...
- **rehype-code-titles**: コードブロックタイトル表示
- **textlint**: 日本語技術文書校正
- **valibot**: スキーマバリデーション
- **Zenn API連携**: `lib/zenn.ts` でZenn記事を取得・表示
