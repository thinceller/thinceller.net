...
2025年6月にリリースされた@opennextjs/cloudflareのバージョン1.2.0の変更内容を見たところ、サーバーのバンドルサイズが約40％減ったようです。[VercelからCloudflareに移行した](2025-03-30-migrate-from-vercel-to-cloudflare)で記載した通り、このブログサイトではOpenNextを使った際のCloudflare Workersへのアップロードサイズが3MBを超えており、Freeプランではアップロードができない状況でした。

1.2.0にアップグレードしたところ、実際にアップロードサイズが約40％減りました。
...
1.2.0にアップグレードしたところ、アップロードサイズは2MB弱に改善していました。ライブラリのアップグレードのみで約40％を削減できました。
...
@opennextjs/cloudflareの1.2.0の変更内容を見たところ、以下の改善を含んでいました。
...
- [perf: drop `babel` to reduce the server bundle size by vicb · Pull Request #891 · opennextjs/opennextjs-aws](https://github.com/opennextjs/opennextjs-aws/pull/891)
  ...
  - これによりBabelの依存を削除でき、バンドルサイズを削減できた
...
このようにOpenNextによるNext.jsへのパッチ適用によってサーバーのバンドルサイズを約40％削減できたようです。
