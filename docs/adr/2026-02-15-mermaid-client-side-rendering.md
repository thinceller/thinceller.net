...
デメリット：
- mermaidライブラリが大きい（minified後で約3-4MB、gzip後で約900KB-1MB）
- 初回表示時にローディング状態が発生
- 初期バンドルには含まれないが、ダイアグラムページ訪問時には必ずロードされる
...
- セキュリティ: MDXファイルは著者が管理し、PRレビュー経由で変更されるため、Mermaid記法の信頼性は担保されている。Mermaid.jsライブラリ自体は依存関係管理（Renovate）で最新版に保つことでXSS脆弱性リスクを軽減

**より困難になったこと:**
- 初回表示時のCLS（Cumulative Layout Shift）対策が必要
  - 対策: 固定高さ（min-height: 200px）のプレースホルダ表示
  - 注: 200pxを超える大きなダイアグラムでは一定のレイアウトシフトが残存
